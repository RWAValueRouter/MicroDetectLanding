    ...
    <main className="relative min-h-screen overflow-hidden px-5 pb-20 pt-28">
      ...
      <header className="fixed left-0 right-0 top-0 z-50 border-b border-white/10 bg-ink/90 backdrop-blur-xl">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4">
          <Link href={homePath} className="flex items-center gap-3"><span className="relative h-10 w-24 overflow-hidden border border-cyan/15 bg-white shadow-sm"><Image src="/logo/md.jpg" alt="MicroDetect Logo" fill sizes="96px" className="object-cover" priority /></span><span className="text-sm font-semibold text-white md:text-base">{isZh ? "析微探物" : "MicroDetect"}</span></Link>
          <div className="flex items-center gap-3"><Link href={routePrefix ? `${routePrefix}/${lang === "zh" ? "en" : "zh"}/products` : `/${lang === "zh" ? "en" : "zh"}/products`} className="border border-cyan/20 bg-white px-4 py-2 text-sm font-semibold text-cyan">{isZh ? "EN" : "中文"}</Link><Link href={`${homePath}#contact`} className="hidden border border-cyan/35 bg-cyan/10 px-4 py-2 text-sm font-medium text-cyan sm:inline-flex">{isZh ? "联系咨询" : "Contact"}</Link></div>
        </nav>
      </header>
