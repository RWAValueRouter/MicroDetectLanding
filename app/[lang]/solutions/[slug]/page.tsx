...
export function generateStaticParams() {
  return supportedLangs.flatMap((lang) => getSeoPagesByKind("solutions").map((page) => ({ lang, slug: page.slug })));
}
...
export default async function LocalizedSolutionPage({ params }: PageProps) {
  const { lang, slug } = await params;
  const page = getSeoPage("solutions", slug);
